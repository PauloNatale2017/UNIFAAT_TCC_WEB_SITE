...
app.controller("CtrlAlertas", ['$scope', '$http', '$location', '$window', 'blockUI', '$timeout', '$interval',
    function ($scope, $http, $location, $window, blockUI, $timeout, $interval) {
        ...
        $scope.SendSMS = function (Numero, IdUsuario) {
            ...
            $http.post(path + "/Home/EnviarEmailPushSMS?Email=" + Numero + "&IdUsuario=" + IdUsuario).then(function (response) {
        ...
        $scope.SendEmail = function (IdUsuario, _email) {
            ...
            $http.post(path + "/Home/EnviarEmailPush?Email=" + _email + "&IdUsuario=" + IdUsuario).then(function (response) {
                if (response.status === 200) {
                    if (response.data === "null") {
                        alert("RETORNO DO REQUEST NULL");
                    } else {
